...
public class DBofBox : MonoBehaviour
{
    ...
    {
        GameObject.Find("Procedure2Canvas/AWBDA/Image").GetComponent<Image>().sprite = Airwaybill;
    ...
    public void callOption()
    {
        try {
            ...
            for (int x = 0; x < Packageoption.Length; x++)
            {
                ...
                else if (this.gameObject.name.Contains("verpack"))
                {
            ...
            //boxesController.getController().initArrays(AWBoption.Length + Packageoption.Length + Dgdoption.Length);
        }
        catch(Exception e) { 
